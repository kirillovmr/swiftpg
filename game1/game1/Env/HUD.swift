...
class HUD: SKNode {
    ...
    func createHudNodes(screenSize: CGSize) {
         ...
         let coinIcon = SKSpriteNode(texture: coinAtlas.textureNamed("coin_bronze"))
        ...
        coinCountText.fontColor = UIColor.black
         ...
         for index in 0 ..< 3 {
             let newHeartNode = SKSpriteNode(texture: textureAtlas.textureNamed("hud_heartFull"))
             newHeartNode.size = CGSize(width: 35, height: 30)
             // Position the hearts below the coins:
             let xPos = -cameraOrigin.x + CGFloat(index * 45) + 33
